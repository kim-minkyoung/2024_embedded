...
// 인터럽트 우선순위 설정: TIM4(서보모터) > USART2(블루투스) > USART1(시리얼) > ADC(압력 센서)
// 서보모터의 우선 순위를 가장 높게 지정하여, 문이 완전히 열고 닫히는 것을 우선으로 한다.
// LED 표시는 현재 상황을 알려주기만 하는 용도이므로, 문 개폐 동작보다 후순위라고 판단하여 우선 순위를 가장 낮게 지정
...
void TIM4_Configure(void)
{
    ...
    TIM_OC3Init(TIM4, &TIM_OCInitStructure);  // 채널 3번 지정
...
void moveMotor(uint16_t var)
{
    ...
    TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;
    TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
    TIM_OCInitStructure.TIM_Pulse = var;  // 인자로 전달 받은 값에 따라 회전 방향 결정
...
int main(void) {
    ...
    setRGBLED(flag);  // LED 초기 색깔 지정
    
    while (1) {
        // USART 전송
        if (flagUART1 == 1){  // 시리얼 통신
            sendDataUART2(wordFromUART1);
            flagUART1 = 0;
        }
        else if (flagUART2 == 1){  // 블루투스 통신
